...
for(const token of ['0.8.0-pass011','diagnostic-tests','symptom-list','parts-grid','repair-history','roadside-actions','runDiagnostic','repairFault','roadsideTriage','conditionDrivingInput','DIAGNOSTIC BAY','REBUILD / PRESERVE'])assert.ok(html.includes(token),`Missing diagnostics token: ${token}`);
