...
const Town = (() => {
  ...
  const ACTORS=['commuter','school-bus','delivery','public-works','emergency','late-local'];
  ...
  class Director{
    ...
    props(player,time=0){if(!this.enabled)return[];const out=[];const add=(mesh,p,x,y,z,w,h,d,color)=>{const co=Math.cos(p.yaw),si=Math.sin(p.yaw);out.push({mesh,x:p.x+co*x+si*z,y,z:p.z-si*x+co*z,w,h,d,color,yaw:p.yaw});};
      const car=(p,kind)=>{const bus=kind==='bus'||kind==='school-bus',van=['van','delivery','works','public-works','utility','emergency'].includes(kind),color=bus?'#d5ac49':kind==='emergency'?'#c7d5d9':van?'#869d9c':'#6f8295';add('box',p,0,.8,0,1.8,.8,bus?6:van?4.5:3.8,color);add('box',p,0,van?1.65:1.3,.3,1.55,van?1.2:.6,bus?4.7:van?3:1.9,'#314754');for(const x of [-.9,.9])for(const z of [-1.25,1.25])add('box',p,x,.42,z,.26,.65,.7,'#252b2c');for(const x of [-.65,.65])add('box',p,x,1.05,1.95,.25,.18,.12,Math.floor(time*2)%2?'#efad55':'#614c36');if(kind==='emergency'||kind==='works'||kind==='utility'||kind==='public-works')add('box',p,0,2.35,0,.9,.16,.25,Math.floor(time*3)%2?'#ef8b52':'#729fc8');};
